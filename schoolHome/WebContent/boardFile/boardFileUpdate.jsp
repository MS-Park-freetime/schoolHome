...
<% 
	...
	BoardFileDTO boardFile = boardFileDAO.getBoard(num);
	
	if(!userID.equals(boardFile.getUserID())){
		session.setAttribute("messageType", "오류메세지");
		session.setAttribute("messageContent", "접근할 수 없습니다.");
		response.sendRedirect("boardFileList.jsp");
		return;
...
<center><b>글수정</b>
<br>
<form method="post" name="writeform" action=".././boardFileUpdate" enctype="multipart/form-data">
...
<table class="board-table" bgcolor="<%=bodyback_c%>" align="center">
  ...
  <tr>
    ...
    <td class="board-table-content"><h5><%=guest.getUserID() %></h5>
       <input type="hidden" class="board-table-input" name="userID" value="<%=guest.getUserID()%>">
  	   <input type="hidden" class="board-table-input" name="num" value="<%=num%>"></td>
  </tr>
  <tr>
    <td class="board-table-title">제 목</td>
    <td class="board-table-content">
       <input type="text" class="board-table-input" name="boardTitle" value="<%=boardFile.getBoardTitle()%>"></td>
  </tr>
  <tr>
    <td bgcolor="<%=value_c%>" align="center">내 용</td>
    <td>
     <textarea name="boardContent" rows="13" cols="40" class="board-table-textarea"><%=boardFile.getBoardContent()%></textarea> </td>
  </tr>
  <tr>
	...
	<td colspan="2">
		...
		<div class="profile-group">
			...
			<input type="text" class="profile-search-form1" disabled placeholder="<%=boardFile.getBoardFile()%>">
			<span>
...
<tr>      
 <td colspan=2 bgcolor="<%=value_c%>" align="center"> 
  <input class="board-input-button" type="submit" value="글수정" >  
